<template>
  <v-container class="content-container timelines">
    <v-timeline clipped large>
      <template v-for="item in years" transition="scale-transition">
        <div :key="item._id">
          <v-timeline-item large class="white--text mb-12" color="blue">
            <template v-slot:icon>
              <span class="text-darken-1">{{ item._id }}</span>
            </template>
          </v-timeline-item>

          <div :key="item._id + 'detail'">
            <v-timeline-item
              :key="item._id"
            ...
            <v-lazy
              ...
              min-height="120"
              v-for="article in timelines[item._id]"
              :key="article._id"
            >
              <v-timeline-item color="green lighten-1" right small>

  ...
  <div class="_page p2 pt1">
    ...
    <div class="table mt2">
      ...
      <el-table :data="tableData" v-loading="loading" border style="width: 100%" size="small" cell-class-name="p0">
        ...
        <el-table-column prop="title" header-align="center" show-overflow-tooltip label="标题" max-width="200">
        </el-table-column>

        <el-table-column prop="summary" header-align="center" show-overflow-tooltip label="简介" max-width="200">
        </el-table-column>
        ...
        <el-table-column prop="tags" align="center" label="标签" min-width="100">
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.tags">
              <el-tag size="mini" v-for="(item, index) in scope.row.tags" :key="index">{{ item.tag ? item.tag.name : '-'
              }}</el-tag>
            </div>
          </template>
        </el-table-column>

        <el-table-column prop="commentCount" align="center" show-overflow-tooltip label="评论数量" max-width="20">
        </el-table-column>

        <el-table-column prop="browseCount" align="center" show-overflow-tooltip label="浏览数量" max-width="20">
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="180">
          <template slot-scope="scope">
            ...
            <template v-if="scope.row.status == 'VerifySuccess'">
              <el-button size="mini" type="primary" v-if="scope.row.up != 'Up'" plain
                @click="handleChangeUpStatus(scope.row, 'Up')">设为置顶</el-button>
              <el-button size="mini" v-else plain @click="handleChangeUpStatus(scope.row, 'Default')">取消置顶</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="文章状态设置" :visible.sync="dialogFormVisible" width="30%">
      <div class="tc">
        ...
        <el-radio-group v-model="currentRadio" size="small">
          <el-radio-button v-for="(item, index) in statusList" :key="index" :label="item.value" >{{ item.name }}</el-radio-button>
        </el-radio-group>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false" size="small">取 消</el-button>
        <el-button type="primary" @click="handleChangeRoleCofirm" size="small" :loading="changStatusLoading">确 定
        </el-button>
      </div>
    </el-dialog>
  </div>
